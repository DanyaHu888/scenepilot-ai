(function () {
  ...
  const sampleScene = "At 5:30 a.m., Mara waits alone at a rural bus stop as fog lifts off the road. A bus arrives, but she stays seated until she notices a child's red scarf caught on the rear gate. She runs after it. The mood is quiet and tense, with a small turn toward hope at the end.";
  let stageTimers = [];
  ...
  function setStageState(activeIndex, completeAll) {
    stages.forEach(function (stage, index) {
      stage.classList.toggle("is-active", !completeAll && index === activeIndex);
  ...
  function startWorkflow() {
    ...
    workflowIntro.textContent = "Building a plan against your brief and day limits.";
  ...
  function completeWorkflow() {
    ...
    workflowIntro.textContent = "The returned plan is ready for a practical production review.";
  }
  ...
  function showWorkflowError() {
    ...
    workflowIntro.textContent = "The plan could not be completed. Your brief is still here to retry.";
  }

  function setLoading(isLoading) {
    generateButton.disabled = isLoading;
    generateButton.classList.toggle("is-loading", isLoading);
    generateButton.querySelector(".button-label").textContent = isLoading ? "Building plan" : "Generate shoot plan";
    generateButton.querySelector(".button-arrow").style.display = isLoading ? "none" : "";
  ...
  function validate() {
    ...
    if (!value) {
      sceneError.textContent = "Add a scene description before generating a plan.";
      sceneField.setAttribute("aria-invalid", "true");
      sceneField.focus();
      return false;
    }
    if (value.length > 2000) {
      sceneError.textContent = "Keep the scene description under 2,000 characters.";
      sceneField.setAttribute("aria-invalid", "true");
      sceneField.focus();
      return false;
    }
  ...
  function showError(message) {
    errorMessage.textContent = message || "Check your connection and try again.";
  ...
  function renderList(value, emptyElement, listElement) {
    ...
    listElement.innerHTML = values.map(function (item) {
  ...
  function renderBreakdown(breakdown) {
    ...
    document.getElementById("breakdown-grid").innerHTML = keys.map(function (key) {
      const values = asArray(source[key]);
      const content = values.length
        ? "<ul class=\"detail-list\">" + values.map(function (item) {
          return "<li>" + escapeHtml(typeof item === "object" ? JSON.stringify(item) : item) + "</li>";
        }).join("") + "</ul>"
        : "<p class=\"detail-empty\">Not provided</p>";
      return "<article class=\"result-card breakdown-card\"><h3>" + breakdownLabels[key] + "</h3>" + content + "</article>";
    }).join("");
  }
  ...
  function renderShots(shots) {
    ...
    document.getElementById("shot-count").textContent = values.length + (values.length === 1 ? " shot" : " shots");
    body.innerHTML = values.map(function (shot, index) {
      const number = displayText(shot.shot_number, index + 1);
      return "<tr>" +
        "<td>" + escapeHtml(number) + "</td>" +
        "<td><strong>" + escapeHtml(displayText(shot.shot_type, "Coverage")) + "</strong><small>" + escapeHtml(displayText(shot.description, "No description")) + "</small></td>" +
        "<td>" + escapeHtml(displayText(shot.equipment, "—")) + "</td>" +
        "<td>" + escapeHtml(displayText(shot.crew_required, "—")) + "</td>" +
        "<td>" + escapeHtml(formatMinutes(shot.estimated_minutes)) + "</td>" +
        "<td>" + escapeHtml(formatMoney(shot.estimated_cost)) + "</td>" +
        "</tr>";
    }).join("");
  ...
  function renderSchedule(schedule) {
    ...
    list.innerHTML = values.map(function (item, index) {
      const shotLabel = item.shot_number !== undefined ? "Shot " + item.shot_number : "Shot " + (index + 1);
      const time = [item.start_time, item.end_time].filter(Boolean).join(" — ");
      return "<li class=\"schedule-item\">" +
        "<span class=\"schedule-time\">" + escapeHtml(time || "Timing pending") + "</span>" +
        "<span class=\"schedule-shot\">" + escapeHtml(shotLabel) + "<span>" + escapeHtml(displayText(item.description, "Coverage")) + "</span></span>" +
        "</li>";
    }).join("");
    empty.hidden = values.length > 0;
  ...
  function renderPlan(plan) {
    ...
    document.getElementById("scene-summary").textContent = displayText(plan.scene_summary, "No scene summary returned.");
    document.getElementById("total-time").textContent = formatMinutes(plan.total_estimated_time);
    document.getElementById("total-cost").textContent = formatMoney(plan.total_estimated_cost);
    ...
    if (/fail|over|risk|exceed|warning/i.test(statusText)) status.classList.add("is-warning");
    if (/not met|invalid|critical/i.test(statusText)) status.classList.add("is-alert");
    renderBreakdown(plan.scene_breakdown);
    renderShots(plan.shot_list);
    renderSchedule(plan.shooting_schedule);
    renderList(plan.warnings, document.getElementById("warnings-empty"), document.getElementById("warnings-list"));
    renderList(plan.adaptations_made, document.getElementById("adaptations-empty"), document.getElementById("adaptations-list"));
    const shotCount = Array.isArray(plan.shot_list) ? plan.shot_list.length : 0;
    document.getElementById("result-meta").textContent = shotCount ? shotCount + " planned coverage " + (shotCount === 1 ? "shot" : "shots") : "Generated from your scene brief";
  ...
  async function requestPlan() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.detail || data.error || "The planning service returned an error.");
      }
      completeWorkflow();
      renderPlan(data);
  ...
  sampleButton.addEventListener("click", function () {
    ...
    document.getElementById("location-type").value = "exterior";
    document.getElementById("equipment").value = "Cinema camera, tripod, LED kit, sound recorder";
    updateCharCount();
    sceneField.focus();
    sceneField.setSelectionRange(sceneField.value.length, sceneField.value.length);
  });
  document.querySelectorAll(".equipment-chip").forEach(function (chip) {
    chip.addEventListener("click", function () {
      document.getElementById("equipment").value = chip.getAttribute("data-equipment") || "";
      document.querySelectorAll(".equipment-chip").forEach(function (other) { other.classList.toggle("is-selected", other === chip); });
